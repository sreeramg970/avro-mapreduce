...
import com.topsoft.botspider.util.AvroUtils;
...
public class MapAvroFile {
  ...
  public static class Writer<K,V> implements java.io.Closeable {
    ...
    private void checkKey(K key) throws IOException {
      ...
      new GenericDatumWriter<K>(keySchema)
          .write(key, new EncoderFactory().directBinaryEncoder(outBuf,null));
      
      inBuf.reset(outBuf.getData(), outBuf.getLength());
      lastKey = new GenericDatumReader<K>(keySchema).read(key,
          df.directBinaryDecoder(inBuf, null));
    }
  }
